...
public class Addition_normal extends Activity {
    ...
    private void checkAnswerTrue() {
        ...
        if (correct == true) {
            ...
            AlertDialog.Builder Success = new AlertDialog.Builder(Addition_normal.this);
            Success.setTitle("Ваш ответ Правильный ");
            Success.show();
            mScore++;
            mPoints++;
        } else {
            AlertDialog.Builder Failure = new AlertDialog.Builder(Addition_normal.this);
    ...
    void updateQuestion(){
        if (mPoints == questions_number) {
            ...
            alert.show();
            Intent intent = new Intent(Addition_normal.this, ResultActivity.class);
    ...
    public ArrayList<Integer> questions_generation(){
        ...
        for (int i = 0; i < 10; i++) {
            values.add((int) (Math.random()*100 + 100));
        }
        ArrayList<Integer> qwerty = new ArrayList<>();
        int a = values.get(1);
        int b = values.get(2);
        int c = (int) Math.round(Math.random() * 100 + 100);
        qwerty.add(a);
        qwerty.add(b);
        qwerty.add(c);
        return qwerty;
