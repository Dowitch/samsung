...
public class Addition_medium extends Activity {
    ...
    private void checkAnswerTrue() {
        ...
        if (correct == true) {
            ...
            AlertDialog.Builder Success = new AlertDialog.Builder(Addition_medium.this);
            Success.setTitle("Ваш ответ Правильный ");
            Success.show();
            mScore++;
            mPoints++;
        } else {
            AlertDialog.Builder Failure = new AlertDialog.Builder(Addition_medium.this);
    ...
    void updateQuestion(){
        if (mPoints == questions_number) {
            ...
            alert.show();
            Intent intent = new Intent(Addition_medium.this, ResultActivity.class);
    ...
    public ArrayList<Integer> questions_generation(){
        ...
        int a = values.get(1);
        int b = values.get(2);
        int c = (int) Math.round(Math.random() * 200 + 150);
        qwerty.add(a);
        qwerty.add(b);
        qwerty.add(c);
        return qwerty;
