...
public class Multiplication_normal extends Activity {
    ...
    private void checkAnswerTrue() {
        ...
        if (correct == true) {
            ...
            AlertDialog.Builder Success = new AlertDialog.Builder(Multiplication_normal.this);
            Success.setTitle("Ваш ответ Правильный ");
            Success.show();
            mScore++;
            mPoints++;
        } else {
            AlertDialog.Builder Failure = new AlertDialog.Builder(Multiplication_normal.this);
    ...
    void updateQuestion(){
        if (mPoints == questions_number) {
            ...
            alert.show();
            Intent intent = new Intent(Multiplication_normal.this, ResultActivity.class);
    ...
    public ArrayList<Integer> questions_generation(){
        ...
        for (int i = 0; i < 10; i++) {
            values.add(rand.nextInt(25) + 1);
        }
        ArrayList<Integer> qwerty = new ArrayList<>();
        int a = values.get(1);
        int b = values.get(2);
        int c = (int) (2 + Math.random()*50);
        qwerty.add(a);
        qwerty.add(b);
        qwerty.add(c);
        return qwerty;
